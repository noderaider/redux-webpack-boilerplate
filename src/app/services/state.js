...
export function loadState(stateKeys = keyNames) {
  let state = {}
...
export function removeState(stateKeys) {
  if(!stateKeys)
...
function getSerializer({ jsonProps, concatProps, base64Props }) {
  ...
  return state => serializers.reduce((mutated, serialize) => serialize(mutated), state)
}
...
function getDeserializer({ jsonProps, concatProps, base64Props, defaultProps = noop() }) {
  ...
  return serialized => typeof serialized === 'undefined' ? defaultProps : deserializers.reduce((mutated, deserialize) => deserialize(mutated), serialized)
}
