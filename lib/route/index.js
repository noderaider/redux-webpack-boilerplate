...
var log = (0, _config.createServerLogger)('routes');
//const cors = getCors()
...
var serveResource = function serveResource(_ref2) {
  ...
  if (assetType === 'static' && !extension) {
    ...
    if (flags.minify && gzipExtensionFilter.test(extension)) {
      ...
      log.debug('serving gzipped resource => path[' + _path + '], headers[' + JSON.stringify(_headers) + ']');
      return { path: _path, headers: _headers };
    } else {
      var _path2 = '/' + assetPath + '/' + assetName;
      log.debug('serving resource => path[' + _path2 + '], headers[' + JSON.stringify(_headers) + ']');
...
var configureRouter = function configureRouter(app) {
  ...
  router.use((0, _serveFavicon2.default)((0, _path3.join)(STATIC_ROOT, 'favicon.ico')));
