...
var startServer = function startServer(app, _ref3) {
  ...
  if (scheme === 'https') {
    ...
    var _tls$passphrase = tls.passphrase;
    var passphrase = _tls$passphrase === undefined ? process.env.PASSPHRASE : _tls$passphrase;

    _chai.assert.typeOf(pfxName, 'string', 'pfxName must be a string filename');
    _chai.assert.typeOf(passphrase, 'string', 'passphrase must be passed or set in environment variable PASSPHRASE');

    return (0, _tls.readPfx)({ filename: pfxName, passphrase: passphrase }).then(function (opts) {
